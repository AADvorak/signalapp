  ...
  watch: {
    model(newValue) {
      if (newValue) {
        ...
        this.changed && this.bus && this.bus.emit(DataViewerEvents.USER_ROLES_MENU_CLOSED_ROLES_CHANGED)
      }
    }
  },
  methods: {
    ...
    async checkBoxStateChanged(role) {
      ...
      if (response.ok) {
        this.changed = true
        this.bus && this.bus.emit(DataViewerEvents.NEW_USER_ROLES, {userId: this.user.id, roles: response.data})
      }
    },
